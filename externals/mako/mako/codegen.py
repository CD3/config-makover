...
class _GenerateRenderMethod(object):
    ...
    def __init__(self, printer, compiler, ignore_expression_errors, node):
        self.printer = printer
        self.compiler = compiler
        self.node = node
        self.identifier_stack = [None]
        self.in_def = isinstance(node, (parsetree.DefTag, parsetree.BlockTag))
        self.ignore_expression_errors = ignore_expression_errors
        ...
        if defs is not None:
            for node in defs:
                _GenerateRenderMethod(printer, compiler, ignore_expression_errors, node)
    ...
    def visitExpression(self, node):
        ...
        if len(node.escapes) or \
            ...
            ibeg = s.find('(') 
            iend = s.rfind(')')
            if self.ignore_expression_errors:
              self.printer.writelines(
                                      "try:"
                                     ,"__M_writer(%s)" % s
                                     ,"except:"
                                     ,"__M_writer(%s)" % (s[0:ibeg+1] + "'''${" + s[ibeg+1:iend] + "}'''" + s[iend:])
                                     ,""
                                     )
            else:
